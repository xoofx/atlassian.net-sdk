...
namespace Atlassian.Jira
{
    ...
    public class ProjectVersionsEditableResource
    {
        ...
        /// <summary>
        /// Gets all versions for the current project (cached);
        /// </summary>
        public IEnumerable<ProjectVersion> Get()
        {
            return _jira.GetProjectVersions(_project.Key);
        }
        ...
        public Task<IEnumerable<ProjectVersion>> GetAsync(CancellationToken token = default(CancellationToken))
        {
            return this._jira.RestClient.GetProjectVersionsAsync(_project.Key, token);
        }
        ...
        public IPagedQueryResult<ProjectVersion> GetPaged(int startAt = 0, int maxResults = 50)
        {
            return ExecuteAndGuard(() => GetPagedAsync(startAt, maxResults).Result);
        }
        ...
        /// <param name="maxResults">How many results on the page should be included. Defaults to 50.</param>
        /// <param name="token">Cancellation token for this operation.</param>
        public async Task<IPagedQueryResult<ProjectVersion>> GetPagedAsync(int startAt = 0, int maxResults = 50, CancellationToken token = default(CancellationToken))
        {
        ...
        /// <param name="projectVersion">Information of the new version.</param>
        /// <param name="token">Cancellation token for this operation.</param>
        public async Task<ProjectVersion> AddAsync(ProjectVersionCreationInfo projectVersion, CancellationToken token = default(CancellationToken))
        {
            var settings = await _jira.RestClient.GetSerializerSettingsAsync(token).ConfigureAwait(false);
        ...
        /// <param name="moveAffectedIssuesTo">The version to set fixVersion to on issues where the deleted version is the fix version, If null then the fixVersion is removed.</param>
        /// <param name="token">Cancellation token for this operation.</param>
        public async Task DeleteAsync(string versionName, string moveFixIssuesTo = null, string moveAffectedIssuesTo = null, CancellationToken token = default(CancellationToken))
        {
            var versions = await this.GetAsync(token).ConfigureAwait(false);
            var version = versions.First(v => v.Name.Equals(versionName, StringComparison.OrdinalIgnoreCase));
